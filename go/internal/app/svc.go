	...
	for {
		...
		if err := s.tryConnect(); err != nil {
			if firstFail {
				...
				notify(s.cfg.NtfyURL, s.cfg.ClickURL, "Бот не подключился", "Не удалось подключиться к серверу TeamTalk", []string{"warning"}, 4)
				failedAt = time.Now()
...
func (s *Service) onEvent(e tt.Event) {
	...
	case tt.EvUserLoggedIn:
		...
		notify(s.cfg.NtfyURL, s.cfg.ClickURL, "Подключился", nick+" ("+user+"), "+mskClock(), []string{"green_circle"}, 3)
	...
	case tt.EvAccountAdded:
		...
		if username != "" && !strings.EqualFold(username, s.cfg.BotUsername) {
			...
			notify(s.cfg.NtfyURL, s.cfg.ClickURL, "Аккаунт создан", "Создан аккаунт «"+username+"»", []string{"tada"}, 3)
		}
	case tt.EvUserKicked:
		...
		notify(s.cfg.NtfyURL, s.cfg.ClickURL, "Бота выкинули", "Бота выкинули с сервера", []string{"boot"}, 4)
	}
}
...
func (s *Service) onMessage(e tt.Event) {
	...
	if kind == "private" {
		notify(s.cfg.NtfyURL, s.cfg.ClickURL, "ЛС от "+from, content, []string{"speech_balloon"}, 4)
	}
}
