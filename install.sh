...
#   5. Creates and enables ~/.config/systemd/user/<service>.service
#        ExecStart: ~/.local/bin/uv run python <repo>/main.py
#
# Usage:
...
if ! loginctl show-user "$USER" 2>/dev/null | grep -q "Linger=yes"; then
    echo -e "${YELLOW}  NOTE: user linger is OFF — service stops on logout.${NC}"
    echo -e "  To keep it running:  ${CYAN}sudo loginctl enable-linger $USER${NC}"
    echo
fi
...
echo -e "    3. Logs        : ${CYAN}journalctl --user -u $SERVICE -f${NC}"
echo
